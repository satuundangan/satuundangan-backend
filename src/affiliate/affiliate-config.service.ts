...
const SYSTEM_KEYS = ['minWithdrawalAmount', 'clearingPeriodDays', 'inactivityDowngradeMonths'] as const;
...
export class AffiliateConfigService {
  ...
  async updateTierConfig(tier: string, dto: UpdateTierConfigDto): Promise<TierConfig> {
    const allowed = Object.values(AffiliateTier) as string[];
    if (!allowed.includes(tier)) {
      throw new BadRequestException(`Unknown tier '${tier}'. Allowed: ${allowed.join(', ')}`);
    }
    const config = await this.tierRepo.findOne({ where: { tier: tier as AffiliateTier } });
    if (!config) {
      throw new NotFoundException(`TierConfig for ${tier} not found`);
    }
    if (dto.minSales !== undefined) config.minSales = dto.minSales;
    if (dto.commissionRate !== undefined) config.commissionRate = dto.commissionRate;
  ...
  async updateSystemConfig(dto: UpdateSystemConfigDto): Promise<Record<SystemKey, number>> {
    const updates: Array<{ key: SystemKey; value: number | undefined }> = [
      ...
      { key: 'inactivityDowngradeMonths', value: dto.inactivityDowngradeMonths },
    ];
    for (const u of updates) {
      if (u.value === undefined) continue;
      const existing = await this.systemRepo.findOne({ where: { configKey: u.key } });
      if (existing) {
        existing.configValue = String(u.value);
        await this.systemRepo.save(existing);
      } else {
        await this.systemRepo.save({ configKey: u.key, configValue: String(u.value), description: null });
      }
